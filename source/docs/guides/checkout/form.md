...
- **Allow Price Modification** - Show and allow the user to edit the amount they they donate or pay.
- **Include Invoice/Order** - Show and allow the user to edit the invoice/order number.
- **Include Card Selection** - Allow the user to select their card brand. This field is largely redundant field. We recommend that it is excluded.
...
You can either pass redirect URLs as a parameter in Checkout URL (see below), or you can set it in the [Member Area](https://web.na.bambora.com). Click **administration**, then **account settings**, and then **order settings**. Under **Transaction Response Pages**, you can enter for Approval Redirect and Decline Redirect respectively.
